...
namespace orca_base
{
  ...
  bool Mission::advance(const std::chrono::milliseconds dt, const PoseStamped &curr, Acceleration &u_bar)
  {
    if (planner_->segments_[segment_idx_]->advance(dt.count() / 1000.0, curr.pose, u_bar, error_)) {
      return true;
    }
