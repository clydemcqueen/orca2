...
namespace orca_base
{
                      ...
                      mw::Acceleration &a0, mw::Twist &v1)
  {
    ...
    p1.pose() = p0.pose() + mw::Pose(xf * xy_sync.d_ramp, yf * xy_sync.d_ramp, z_sync.d_ramp, yaw_sync.d_ramp);
    p2.pose() = p1.pose() + mw::Pose(xf * xy_sync.d_run, yf * xy_sync.d_run, z_sync.d_run, yaw_sync.d_run);
  ...
  PoseSegmentBase::PoseSegmentBase(const AUVContext &cxt, uint8_t type, mw::PoseStamped start, mw::Pose goal) :
  ...
  Pause::Pause(const AUVContext &cxt, const mw::PoseStamped &start, const rclcpp::Duration &pause_duration) :
    PoseSegmentBase{cxt, orca_msgs::msg::MissionState::PAUSE, start, start.pose()}, pause_duration_{pause_duration}
  {}
  ...
  Trap2::Trap2(const AUVContext &cxt, uint8_t type, const mw::PoseStamped &start, const mw::Pose &goal) :
    PoseSegmentBase{cxt, type, start, goal}
  {
    p0_ = start;
    // plan_sync will set p3_.header().t()
    p3_.pose() = goal;
  ...
  std::shared_ptr<Trap2> Trap2::make_vertical(const AUVContext &cxt, mw::PoseStamped &plan, double z)
  {
    auto goal = plan.pose();
    goal.z() = z;
    auto result = std::make_shared<Trap2>(cxt, orca_msgs::msg::MissionState::POSE_VERTICAL, plan, goal);
    plan.pose() = goal;
    plan.header().t() = plan.header().t() + result->duration();
    return result;
  }

  std::shared_ptr<Trap2> Trap2::make_rotate(const AUVContext &cxt, mw::PoseStamped &plan, double yaw)
  {
    auto goal = plan.pose();
    goal.yaw(yaw);
  ...
  std::shared_ptr<Trap2> Trap2::make_line(const AUVContext &cxt, mw::PoseStamped &plan, double x, double y)
  {
    auto goal = plan.pose();
    goal.x() = x;
    goal.y() = y;
  ...
  std::shared_ptr<Trap2> Trap2::make_pose(const AUVContext &cxt, mw::PoseStamped &plan, const mw::Pose &goal)
  {
