...
bool RotateMotion::init(const OrcaPose &goal, OrcaOdometry &plan)
{
  ...
  {
    RCLCPP_ERROR(logger_, "Can't init rotate motion: x, y and z must be the same");  // TODO need logger
    return false;
  }
...
bool LineMotion::init(const OrcaPose &goal, OrcaOdometry &plan)
{
  ...
  {
    RCLCPP_ERROR(logger_, "Can't init line motion: z and yaw must be the same");
    return false;
  }
...
bool ArcMotion::init(const OrcaPose &goal, OrcaOdometry &plan)
{
  ...
  {
    RCLCPP_ERROR(logger_, "Can't init arc motion: z must be the same");
    return false;
  }
  ...
  if (arc_.radius_ < 1)
  {
    RCLCPP_ERROR(logger_, "Can't init arc motion: radius is too small");
    return false;
  }
...
bool VerticalMotion::init(const OrcaPose &goal, OrcaOdometry &plan)
{
  ...
  {
    RCLCPP_ERROR(logger_, "Can't init vertical motion: x, y and yaw must be the same, goal.z must be negative");
    return false;
  }
