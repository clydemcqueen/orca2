  ...
  yaw_controller_{true, 0.007, 0, 0}
{
  ...
  RCLCPP_INFO(get_logger(), "orca_base initialized");
}
...
void OrcaBase::goalCallback(const geometry_msgs::msg::PoseStamped::SharedPtr msg)
{
  ...
  if (mode_ != orca_msgs::msg::Control::DISARMED && barometer_ready_ && imu_ready_) {
    ...
    RCLCPP_INFO(get_logger(), "Start mission at (%g, %g, %g), goal is (%g, %g, %g), heading %g",
...
int main(int argc, char **argv)
{
  ...
  rclcpp::Rate r(orca_base::SPIN_RATE);  // TODO rclcpp::Rate doesn't honor /clock, possibly fixed in Dashing
  while (rclcpp::ok()) {
    // Do our work
    node->spinOnce();
