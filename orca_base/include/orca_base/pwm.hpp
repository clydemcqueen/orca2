...
// Range with deadzone
  ...
  {
    return clamp(static_cast<uint16_t>(THRUST_STOP_PWM +
                                       (effort > THRUST_STOP ? THRUST_DZ_PWM : (effort < THRUST_STOP ? -THRUST_DZ_PWM
                                                                                                     : 0)) +
                                       std::round(effort * THRUST_RANGE_PWM)), THRUST_FULL_REV_PWM,
                 THRUST_FULL_FWD_PWM);
  }

  constexpr const double pwm_to_effort(const uint16_t pwm)
  {
    return static_cast<double>(pwm - THRUST_STOP_PWM +
