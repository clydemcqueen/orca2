...
The high-level controllers calculate the error and pass it to the PID controllers.
There are 2 high-level controllers:

The **PoseController** uses 4 PID controllers (x, y, z, yaw)
to control motion during normal operation.

The **ObservationController** uses 3 PID controllers (forward, vertical, bearing)
to control motion during observation-based recovery operations.
...
Updates October 2020 after some field testing:
* The thrust curve is decidedly not linear.
In general the AUV uses just the low end of the curve, and this segment is roughly linear.
The parameter `mdl_thrust_scale` can be used to modify the force to effort calculation.
* There is a dead band using the BlueRobotics Basic ESC R3’s.
This can be set using `mdl_thrust_dz_pwm`.
